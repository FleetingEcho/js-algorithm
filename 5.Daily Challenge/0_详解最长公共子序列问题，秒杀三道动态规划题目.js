...
function dp(s1, i, s2, j) {
	...
	if (i == s1.length) {
		...
		for (; j < s2.length; j++) res += s2.charCodeAt(j) // 删除s2剩余所有元素
		return res
	}
	if (j == s2.length) {
		// 如果 s2 到头了，那么 s1 剩下的都得删除
		for (; i < s1.length; i++) res += s1.charCodeAt(i) // 删除s1剩余所有元素
		return res
	}
	...
	if (s1.charAt(i) == s2.charAt(j)) {
		...
		memo[i][j] = Math.min(s1.charCodeAt(i) + dp(s1, i + 1, s2, j), s2.charCodeAt(j) + dp(s1, i, s2, j + 1))
	}
	return memo[i][j]
}
