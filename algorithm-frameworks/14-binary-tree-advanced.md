...
```mermaid
flowchart TD
    START["二叉树进阶题"] --> ASK{"题目在问什么?"}
    ASK -->|公共祖先| LCA["LCA<br/>后序遍历返回命中节点"]
    ASK -->|保存 / 还原结构| SER["序列化<br/>前序或层序 + 空标记"]
    ASK -->|路径是否存在| PATH1["路径总和 I<br/>递归减 target"]
    ASK -->|列出所有路径| PATH2["路径总和 II<br/>DFS + 回溯 path"]
    ASK -->|路径最大贡献| MAXPATH["最大路径和<br/>后序计算单边贡献"]
    ASK -->|每层可见节点| VIEW["右视图<br/>BFS 每层最后 / DFS 深度首次"]
    ASK -->|展开 / 迭代| STACK["指针重连 / 栈模拟中序"]
```
