...
namespace Leetcode253 {
  //!  写的有点问题，因为cover不到所有的情况！
  // > 应该用优先级队列
  const minMeetingRooms1 = function (intervals: number[][]): number {
    // base case
    ...
    for (let i = 1; i < arr.length; i++) {
      // 先查询
      let temp: string[] = Object.keys(memo).sort((a: any, b: any) => a - b);
      if (temp.length !== 0) {
        let res = false;
        console.log(memo);
  ...
  const minMeetingRooms = function (intervals: number[][]): number {
    if (intervals.length === 0) return 0;
    //按时间顺序排序
  ...
  const minMeetingRoom2 = (intervals: number[][]): number => {
